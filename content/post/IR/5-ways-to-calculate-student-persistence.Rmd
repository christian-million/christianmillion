...
date: '2019-07-17'
categories: institutional_research
draft: yes
featured: no
image:
  caption: ''
...
Once I became comfortable enough with R's syntax, I was quickly able to calculate persistence on the fly. It was so exciting that I took the time to convert it into my first function. It was an early version of my `persistence()` function.

I soon found out there are many approaches to calculating persistence. In this post, I'm hoping to weigh the limitations and benefits of each method.

# What is Student Persistence?

The word 'persistence' can mean many different things, depending on the context. For my purposes, I am interested in "term-to-term" persistence, which measures whether a student in Term A subsequently enrolled in Term B. Term-to-term persistence can mean:
  ...
  - any other combination of terms in sequence -- winter quarters, summer sessions, etc. The word 'term' is used as an institutional agnostic word for "semester", "quarter", "trimester", or what-have-you.
...
From an institutional perspective, we are likely to get more funding when the persistence rate is high. We have more enrollments and more students are likely to earn their degree / certificate (and at a faster rate). Our numbers as an institution look better when we have high persistence. It is important to measure persistence rates for subpopulations, so that if we implement an intervention, we can measure whether or not that intervention effected the likelihood of those students returning.
...
More persistence isn't always better, though. A common example of this is Career Technical Education (CTE) focused students. Students take classes in one term to gain skills that employers are looking for. Once they gain those skills, and maybe get hired, there is no reason to come back the next term to pursue a terminal certificate or degree or transfer. Returning may mean working less, spending more on tuition, accumulating unnecessary units, or anything not conducive to their goals.

# General Approaches to Calculating Persistence

So, the title of this post is a little misleading. While I will show 5 different ways to calculate student persistence, two of them use a similar *approach*, but a different technical implementation. In general though, I've found these approaches useful:

  1. Joins - Joining 2 data.frames. 1 contains the initial term, the other contains the subsequent term.
  ...
  3. Lookup - Similar to Excels VLOOKUP (and Joins, really). Lookup values from initial term in target term.
  
All of these approaches have an important common feature: They create a persistence indicator variable first on the original data, and then separately, we calculate the persistence from that indicator. This is crucial for my work, because I need to look at persistence among many subpopulations. The indicator allows that to be more dynamic. Additionally, I sometimes need to export the resulting data into another software (Like PowerBI), and so the indicator is just one feature of many.
...
In general, this approach takes a single dataframe, ranks the terms, sorts the dataframe by id and term, and then implements a 'lead' style function that finds the next value in the vector. If the current value + 1 is equal to the next, then persistence!
...
calc_persist2 <- function (df, id, rank) {
    ...
    arrange(denseRank) %>%
    mutate(nextrank = lead(denseRank)) %>%
    mutate(persist = case_when(nextrank == denseRank + 1 ~ 1, TRUE ~ 0)) %>% ungroup() %>% select(-nextrank, -denseRank)
```
#### Approach #3
